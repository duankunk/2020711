...
public interface ShoppingService {
	 ...
	 * 参数 Shoppping
	 * 返回值 null
	 * 备注   保存订单订单状态0为购买 1为正在购买 2已购买3取消4为心愿
	 * @param o
	 */
	@RequestMapping("/save")
	public Map<String , Object> save(@RequestParam("status") int status,@RequestParam("userid") int userid,
			@RequestParam("sptiesid") int sptiesid,@RequestParam("quantity") int quantity,
			@RequestParam("dimensions") int dimensions,@RequestParam("scolor") int scolor);
	 ...
	 * 作者 ： kun
	 * 编辑日期 ： 下午4:29:08
	 * updateShopping作用 ： (更新用户购物车或者心愿单一个商品的状态 状态0为购买 1为正在购买 2已购买3取消	4为心愿)
	 ...
	 * 作者 ： kun
	 * 编辑日期 ： 上午10:07:55
	 * updateUidShoppingNum作用 ： (更改数量)
	 * 返回类型 ：Map<String,Object>
	 */
	@RequestMapping("/updateUidNum")
	public Map<String , Object> updateUidShoppingNum(@RequestParam("num") int num,@RequestParam("id") Long id,@RequestParam("userid") int userid);
	/**
	 * 
	 * 作者 ： kun
	 * 编辑日期 ： 下午6:59:30
